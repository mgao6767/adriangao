...
Since $f(a,o)=a+(1-a)o$, the first-order condition of the agent’s objective function with respect to a is given by $a^*=\theta(1-o)\mathbin{/}g$, which implies his effort choice is decreasing in the cost of effort $g$, decreasing in the firm’s organization capital $o$, and increasing in the pay-for-performance sensitivity $\theta$.
...
where $w\equiv -\ln(-\underline{U})\mathbin{/} \eta$ is a constant determined by the agent's reservation utility and his coefficient of constant absolute risk aversion. Substituting in $a^*=\theta(1-o)\mathbin{/}g$, we yield
...
Thus, by substituting $a^*=\theta(1-o)\mathbin{/}g$ into the principal’s objective function $E\left[V-c\right]$, we yield
...
On the other hand, fixing $a^*$, the required pay-for-performance sensitivity is $\theta=(a^* g)\mathbin{/}(1-o)$, which is increasing in organization capital $o$. Thus, to elicit any given level of effort, the incentive compensation must be more high-powered ("fixed target action" as in [Edmans and Gabaix (2016)](https://www.aeaweb.org/articles?id=10.1257/jel.20161153).
