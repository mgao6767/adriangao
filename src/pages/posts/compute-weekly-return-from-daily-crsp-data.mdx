...
<Aside type="tip" title="tl;dr. Take me to the final solution!">
  Surely -> [The
  solution](#2-group-using-aligned-dates-fast-version-with-caveat) is the very
  last code block of this post.
</Aside>
...
that if the given date is already a Friday, we don't go for the next one.
...
Therefore, we modify the code above to produce the final weekly dataset using Friday as the date identifier by keeping `FridayOfWeek` and `mktret`.
...
- Dec30 and Dec31 belong to week 53 of 2019, while the code above will use these
  two days' returns to compute the weekly return and align the date to Jan03 of 2020.
...
<Aside type="caution" title="Caveat">
  ...
  those weeks. To fix this minor issue, simply extand the beginning and ending
  dates beyond your sample period by a few weeks.
</Aside>
