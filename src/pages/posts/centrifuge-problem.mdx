...
This is a simple yet interesting problem, very well illustrated by [Numberphile](https://youtu.be/7DHE8RnsCQ8) and discussed by [Matt Baker's blog](https://mattbaker.blog/2018/06/25/the-balanced-centrifuge/).

The now proved solution is that:

<Aside type="note" title="Solution">
  You can balance $k$ identical test tubes, $1\le k\le n$, in an $n$-hole
  centrifuge if and only if both $k$ and $n-k$ can be expressed as a sum of
  prime divisors of $n$.
</Aside>

An example: balanced 18-hole centrifuge

![18-hole-centrifuge](/images/centrifuge/18-hole-centrifuge.svg)

Another example: balanced 20-hole centrifuge

![20-hole-centrifuge](/images/centrifuge/20-hole-centrifuge.svg)
...
There can be a few optimizations, for example, we can compute only the lower half of $k$:
...
We can use $f[k]$=`True` to represent that $k$ is a linear combination of $n$'s prime divisors. A value $i$ is either itself a prime divisor of $n$ (and thus a linear combination of the prime divisors), or the sum of a $n$'s prime divisor $p$ and $(i-p)$. In the latter case, if $(i-p)$ is a linear combination of $n$'s prime divisors, so is $p+(i-p)=i$.
...
### "6-hole"
    ...
    ```
    ![6-hole-centrifuge](/images/centrifuge/6-hole-centrifuge.svg)
...
### "10-hole"
    ...
    ```
    ![10-hole-centrifuge](/images/centrifuge/10-hole-centrifuge.svg)
...
### "12-hole"
    ...
    ```
    ![12-hole-centrifuge](/images/centrifuge/12-hole-centrifuge.svg)
...
### "24-hole"
    ...
    ```
    ![24-hole-centrifuge](/images/centrifuge/24-hole-centrifuge.svg)
...
### "33-hole"
    ...
    ```
    ![33-hole-centrifuge](/images/centrifuge/33-hole-centrifuge.svg)
