...
<Aside type="note" title="example `docker-compose.yml` for nginx-proxy">
    ```yaml
    version: '3'
...
<Aside type="note" title="Increase upload file size">
...
<Aside type="note" title="example `docker-compose.yml` for WordPress container">
    ```yaml
    version: "3"
