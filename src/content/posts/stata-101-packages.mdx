---
title: Stata 101 - Packages
pubDate: Feb 16, 2023
draft: false
heroImage: /images/stata/antigravity.png
tags:
- Stata
- packages
---
...
<Aside type="tip" title="Previous section">
  [Stata 101 - Introduction](/posts/stata-101-introduction).
</Aside>
...
Specifically, we use the builin command [`ssc`](https://www.stata.com/manuals/rssc.pdf) to install a package named `nicewords` in the first line, and then execute the command `nicewords` in the second line, which randomly prints some nice words.
...
[`estout`](https://github.com/benjann/estout) is also a top 10 Stata package that provides tools to make regression tables. We've seen an example from the [previous post](/posts/stata-101-introduction#save-to-latex). I highly recommend, too!

`outreg2` does a similar job in a simpler way. Yet if we want finner controls `estout` is perhaps better, in my humble opinion.

### `winsor` and `winsor2`

Data is often noisy with extreme values or impossible values recorded by mistake. In some fields of research, we try to mitigate such concern by [winsorization](https://en.wikipedia.org/wiki/Winsorizing). Note that they may yield different results due to their different approaches in determinining percentile values.
