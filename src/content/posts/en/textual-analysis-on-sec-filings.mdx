...
<Aside type="danger" title="warning">
    As per [SEC's policy](https://www.sec.gov/os/accessing-edgar-data), you should limit concurrent requests to below
    10 per second. Proper User-Agent must also be provided in request' header.
...
```python
# Download all missing 8-K filings.
...
if __name__ == "__main__":
    ...
    jobs = [tuple(l.split(",")) for l in data]
    random.shuffle(jobs)
    del data
    ...
    for job in jobs:
        ...
        if os.path.exists(filename):
            ...
            download(job)
            progress.update()
            time.sleep(0.25) # at most 4 requests per second
```
